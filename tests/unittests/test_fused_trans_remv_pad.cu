...
int main() {
    ...
    Tensor in(Device::GPU, type, {batch_size, head_num, max_seq_len, head_size}, d_in);
    Tensor in_pad(Device::GPU, type_pad, {num_tokens}, d_padding_offset);
    Tensor out(Device::GPU, type, {num_tokens, head_num, head_size}, d_out);
