#include "src/layers/context_decoder.h"
template<typename T>
LlamaContextDecoder::allocForForward(LLaMAAttentionDynParams& params)
{
    ...
    attention_mask->data = allocator_->reMalloc(attention_mask->data, sizeof(T) * batch_size * max_q_len * max_k_len, false);
    padding_offset->data = allocator_->reMalloc(padding_offset->data, sizeof(int) * batch_size * max_q_len, false);
    cum_seqlens->data     = allocator_->reMalloc(cu_seqlens->data, sizeof(int) * (batch_size + 1), false);
   
}
LlamaContextDecoder::free()
{
...
LlamaContextDecoder::forward(TensorMap& input_tensors, const std::vector<LlamaLayerWeight*>& layerWeights, TensorMap& output_tensors)
{
...
//     };
    ...
    launchFusedAddBiasResidualRMSNorm(nullptr, //in, [num tokens, hidden_units]
    ...
    for(int layer_id = 0; layer_id < num_layer; layer_id++) {
        ...
        ctxAttn->forward(ctx_attn_inputs, ctx_attn_outputs, *layerWeights[layer_id], ctxAttn->attn_static_params, dyn_params);
        //decoder_output += decoder_input
        launchFusedAddBiasResidualRMSNorm(decoder_input, //in residual, [num tokens, hidden_units]
                                        decoder_output, //in&out, [num tokens, hidden_units]
                                        layerWeights[layer_id]->self_attn_weight.output.bias, //bias
        ...
        auto gamma = layer < num_layer - 1 ? layerWeights[layer + 1]->attn_norm_weight->gamma :
                                                     (float*)input_tensors["output_norm_weight"].data;//llamaweight->output_norm_weight
        launchFusedAddBiasResidualRMSNorm(decoder_input, //in, [num tokens, hidden_units]
                                        decoder_output, //in&out, [num tokens, hidden_units]
                                        layerWeights[layer_id]->ffn_weight.output.bias, //bias
