...
template<typename T>
LLaMAContextAttentionLayer::allocForForward(LLaMAAttentionDynParams params) {
    ...
    k_cache_buf->data = allocator->Malloc(
        k_cache_buf->data, 2 * sizeof(T) * batch_size * head_num * max_k_len * head_size);
...
void LLaMAContextAttentionLayer::freeBuffer(){
    ...
    allocator->free((void**)(&q_buf_w_pad->data));
    allocator->free((void**)(&k_cache_buf->data));
    allocator->free((void**)(&v_cache_buf->data));
    allocator->free((void**)(&qk_buf->data));
    allocator->free((void**)(&qkv_buf_w_pad->data));
    allocator->free((void**)(&qkv_buf_wo_pad->data));
}
...
{   //Can we wrapper the output buf pointer into tensor also?
    ...
    launchAddFusedQKVBiasTransposeAndRoPE(&q_buf_w_pad, &k_buf_w_pad, &v_buf_w_pad, &qkv_buf_wo_pad
                                        &qkv_bias, &padding_offset, &history_length, &input_length);
