                               ...
                               int vocab_size)
{
    ...
    topk_val[offset] = expf(topk_val - max_val);
    ...
    if(tid == 0) {
        ...
        curandState_t state;
        curand_init(rand_num, bid, 0, state);// not sure rand_num's type is suitable here or not
        ...
        is_finished[bid] = output[bid] == end_id ? 1 : 0;
    }
}
